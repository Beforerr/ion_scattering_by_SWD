...
$$
\begin{aligned}
f_1(z)\to \frac{1}{2} \cos β \ \left(\text{Ci}\left(β +β \tanh \left(\frac{z}{L}\right)  \right)-\text{Ci}\left(β -β  \tanh \left(\frac{z}{L}\right)\right)\right)+ \frac{1}{2} \sin β \ \left(\text{Si}\left(β +β \tanh \left(\frac{z}{L}\right) \right)-\text{Si}\left(β -β  \tanh \left(\frac{z}{L}\right)\right)\right),
\\
f_2(z)\to \frac{1}{2}  \sin β \ \left(\text{Ci}\left(β +β \tanh \left(\frac{z}{L}\right)  \right)+\text{Ci}\left(β -β  \tanh \left(\frac{z}{L}\right)\right)\right) - \frac{1}{2} \cos β \ \left(\text{Si}\left(β +β \tanh \left(\frac{z}{L}\right)  \right)+\text{Si}\left(β -β  \tanh \left(\frac{z}{L}\right)\right)\right)
\end{aligned}
$$

We introduce a dimensionless Hamiltonian $\tilde{H} = H / h$ by normalizing $(x, z) = (\tilde{x} L + \frac{c p_y}{q B_z}, \tilde{z} L)$ and $(p_x, p_z) = p (\tilde{p_x}, \tilde{p_z})$ where $p = \frac{q L B_t}{c}$ and $h = \frac{q^2 L^2 B_t^2}{m c^2}$.

$$
\tilde{H}\to \frac{1}{2} \left(\left(\tilde{p_x}-f_1(z)\right)^2+\left(\tilde{x} \cot θ + f_2(z)\right)^2+\tilde{p_z}^2\right)
$$

It is important to note that this is an exact Hamiltonian of the particle dynamic. We do not expand it near the neutral plane $z=0$ like previous study employing the Hamiltonian formalism. This Hamiltonian also include the rotation effect $β$. Expand the Hamiltonian near the current sheet to the third order and assume that $β=1$ we could reduce to the Hamiltonian form in @artemyevSuperfastIonScattering2020 where $κ = \cot θ$. In this study, we quantitatively investigate the role of $β$ and particle energy $E$ in the pitch-angle scattering efficiency. In the following discussion, the tilde notation over the dimensionless variable is omitted for simplicity.

# Violation of the adiabatic invariant

For discontinuities where $B_n / B_t ≪ 1$, the variables $(κ x, p_x)$ evolve significantly more slowly compared to $(z, p_z)$. Assuming that $x$ and $p_x$ are effectively frozen, the Hamiltonian describes a periodic motion within the $(z, p_z)$ plane, governed by the effective potential energy $U(z) = H - p_z^2$. For slow variations in $(κ x, p_x)$, the generalized magnetic moment $I_z = (2 π)^{-1} ∮ p_z dz $ is conserved as an adiabatic invariant, with changes occurring only in response to substantial trajectory reconfigurations. 

The Hamiltonian captures two distinct types of ion motion within the $(z, p_z)$ plane. As $(κ x, p_x)$ evolve slowly, the corresponding trajectory in the $(z, p_z)$ plane undergoes a gradual transformation. Particles may transition from one type of motion to another via a separatrix crossing, which results in a jump in $I_z$ of approximately $\sim 1$, attributed to the geometrical destruction of adiabatic invariance. Concurrently, the projection of the particle’s phase point onto the $(κ x, p_x)$ plane lies along the so-called uncertainty curve, which demarcates regions corresponding to different types of motion in the $(z, p_z)$ plane. Importantly, this $2π I_z$ jump corresponds to the difference in areas enclosed by the separatrix in the $(z, p_z)$ plane and is independent of $κ$.
...
Due to the symmetry of ion motion about the $p_z = 0$ line, at the saddle point $z = z_c, p_z = 0$ of the separatrix, the potential energy $U$ reaches a local maximum along the $z$-direction, satisfying $∂ U/∂ z = 0$ and $∂^2 U/∂ z^2 < 0$. These conditions allow the expression of the slow variables along the uncertainty curve in the $(κ x, p_x)$ plane as functions of $z_c$. When a particle trajectory crosses the $z = 0$ plane, the crossing point is distributed within a circular region defined by $(p_x - f_1(0))^2 + (\kappa x + f_2(0))^2 = 2H$. The likelihood of the trajectory intersecting the uncertainty curve is therefore proportional to the separatrix length $L_{\text{sep}}$, normalized by the square root of the particle energy $H = E$.
...
To quantitatively understand how particles are scattered by solar wind discontinuities, we conducted extensive test particle simulations using the distribution of real solar wind parameters at 1 AU. As indicated by the dimensionless Hamiltonian, the key parameters influencing particle dynamics are the two angles $θ, β$ and the normalized particle energy $E/h$. Since our primary focus is on protons, we assume a particle mass equal to $m_p$ (the proton mass) and a charge equal to the elementary charge $q$. For a given particle energy, the normalized particle energy can be determined from the magnetic field configuration, specifically $B_t = B_0 \sin θ$ and the characteristic scale $L$. Using a simple transformation, $B_t = B_0 \sin θ$, the parameters critical for investigating ion scattering are $θ$, $β$, and $v_0 = Ω_0 L = q B_0 L / (c m_p)$, where $Ω_0$ is the cyclotron frequency corresponding to the reference magnetic field $B_0$.
