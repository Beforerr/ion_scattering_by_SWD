...
using Match
using CurrentSheetTestParticle: field_lines, get_gc_func, solve_param
...
begin
    ...
    colorrange(; scale = colorscale) = scale == log10 ? (1.0e-2, 1.0e1) : (0, 5)
...
function pa_pair_hist!(df::AbstractDataFrame, layout, s = :v; layer = pa_layer(s), scales = tm_scale(), axis = w_axis, kwargs...)
    ...
    return sdraw!(layout, data(df) * layer, s => r; scales, axis, kwargs...)
end
...
function sign_label(fig; i1 = 0, i2 = 2)
    ...
    return Label(fig[i2, :], "Right-hand rotation", font = :bold, tellwidth = false)
end
...
function pa_diff_plot!(fig, layer; kwargs...)
    ...
    return draw!(fig, plt; axis, kwargs...)
end
...
function plot_gc!(sol, B; color = Makie.wong_colors()[2], label = "Guiding Center")
    ...
    return lines!(sol, idxs = (gc_plot, 1, 2, 3, 4, 5, 6); color, label)
end
